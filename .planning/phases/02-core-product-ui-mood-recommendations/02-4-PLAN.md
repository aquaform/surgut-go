...
<task type="auto">
  ...
  <behavior>
    ...
    - renderCard escapes every text field via escHtml() — INCLUDING the CTA href value escHtml(item.event.sourceUrl); renders title, humanized date/time, venue, price (omitted when "Цена не указана"), reason with mood emoji, source name, and a CTA anchor to sourceUrl with target="_blank" rel="noopener noreferrer" reading "Купить билет" for ticketing sources else "Открыть"
    - When item.event.isSeed === true, a .badge.badge--demo "Демо" element is rendered on the card; else when the event's source (matched by sourceName via the sourceStatusByName map from loadSources) has status === "cached", a .badge.badge--cached "Кэш" element is rendered — so no seed/stale card is ever shown unbadged (success criterion 4)
...
<task type="auto">
  ...
  <verify>
    <automated>npm run test -- --coverage 2>&1 | tee /tmp/cov.txt && node -e "const t=require('fs').readFileSync('/tmp/cov.txt','utf8'); if(/fail/i.test(t)&&!/0 failed|failed \\(0\\)/i.test(t)) throw new Error('tests failed'); const m=t.match(/All files\\s*\\|\\s*([0-9.]+)/); if(!m) throw new Error('no coverage summary'); const lines=parseFloat(m[1]); if(lines<80) throw new Error('lines coverage '+lines+'% < 80%'); console.log('coverage ok: '+lines+'%');"
  </verify>
  <acceptance_criteria>
